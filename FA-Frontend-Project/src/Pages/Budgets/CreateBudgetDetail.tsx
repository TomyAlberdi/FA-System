...
export const generatePDF = (budget: CompleteBudget | null) => {
  const doc = new jsPDF();
  ...
  */
  // Budget Number
  doc.setFont("helvetica", "bold");
  doc.text(`Presupuesto: ${budget?.id?.toString().padStart(10, "0")}`, 10, 40);
  /* 
  doc.text(
  ...
  */
  ...
  doc.text(`Importe Total: $ ${budget?.finalAmount}`, rightAlignX, startY + 10);
