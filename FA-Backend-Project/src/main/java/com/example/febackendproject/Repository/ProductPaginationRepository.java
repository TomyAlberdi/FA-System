...
public interface ProductPaginationRepository extends PagingAndSortingRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query("SELECT new com.example.febackendproject.DTO.PartialProductDTO(p.id, p.name, p.measureType, p.measurePrice, p.discountPercentage, p.discountedPrice, '') FROM Product p")
