...
public interface ProductPaginationRepository extends PagingAndSortingRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query("SELECT new com.example.febackendproject.DTO.PartialProductDTO(p.id, p.name, p.disabled, p.measureType, p.measurePrice, p.saleUnit, p.saleUnitPrice, p.measurePerSaleUnit, p.discountPercentage, p.discountedPrice,p.discountedMeasurePrice, '') FROM Product p")
